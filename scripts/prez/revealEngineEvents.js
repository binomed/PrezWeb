...
import {
	BlePrezControler
} from '../sensors/blePrezControler.js';
import {
	VoiceRecognitionControler
} from '../sensors/voiceRecognitionControler.js';
import {
	SpeechSynthesisControler
} from '../sensors/speechSynthesisControler.js';
import {
	ImageCaptureControler
} from '../sensors/imageCapture.js';
...
export class RevealEngineEvents {
	...
	_imageCaptureEvents() {
		...
		Reveal.addEventListener('stop-image-capture-demo', event => {
			if (imageCaptureController) {
				imageCaptureController._destroy();
			}
		});
		Reveal.addEventListener('image-capture-demo', event => {
			imageCaptureController = new ImageCaptureControler();
		});
