...
import {
	MyoControler
} from '../webbluetooth/myoControler.js';
import {
	MBot
} from '../webbluetooth/mbotControler.js';
		...
		document.getElementById("connectMBot").addEventListener('click', _ => {
			...
			mBot.request()
				...
				.then(_ => {
					...
					btnUp.addEventListener('mousedown', _ => {
						mBot.processMotor(-250, 250)
					});
					btnDown.addEventListener('mousedown', _ => {
						mBot.processMotor(250, -250)
					});
					btnLeft.addEventListener('mousedown', _ => {
						mBot.processMotor(250, 250)
					});
					btnRight.addEventListener('mousedown', _ => {
						mBot.processMotor(-250, -250)
					});

					btnUp.addEventListener('mouseup', _ => {
						mBot.processMotor(0, 0)
					});
					btnDown.addEventListener('mouseup', _ => {
						mBot.processMotor(0, 0)
					});
					btnLeft.addEventListener('mouseup', _ => {
						mBot.processMotor(0, 0)
					});
					btnRight.addEventListener('mouseup', _ => {
						mBot.processMotor(0, 0)
					});
